...
COptionsSubMouse::COptionsSubMouse(vgui::Panel *parent) : PropertyPage(parent, NULL)
{
	...
	m_pMouseAccelerationSlider = new CCvarSlider(this, "MouseAccelerationSlider", "#GameUI_MouseAcceleration",
		0.5f, 7.0f, "m_customaccel_exponent", true);

	m_pMouseAccelerationLabel = new TextEntry(this, "MouseAccelerationLabel");
	m_pMouseAccelerationLabel->AddActionSignalTarget(this);
	...
	ConVarRef var2("acceleration");
	if (var2.IsValid())
	{
		float acceleration = var2.GetFloat();

		char buf[64];
		Q_snprintf(buf, sizeof(buf), " %.1f", acceleration);
		m_pMouseAccelerationLabel->SetText(buf);
	}
